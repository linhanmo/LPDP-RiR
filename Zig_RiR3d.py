...
wkv_cuda = None
if "wkv_cuda" in sys.modules:
    wkv_cuda = sys.modules["wkv_cuda"]

if wkv_cuda is None and os.environ.get("RWKV_FORCE_FALLBACK") != "1":
    try:
        from torch.utils.cpp_extension import load

        base_path = Path(__file__).resolve().parent
        sources = [
            str(base_path / "cuda" / "wkv_op.cpp"),
            str(base_path / "cuda" / "wkv_cuda.cu"),
        ]
        wkv_cuda = load(
            name="wkv",
            sources=sources,
        ...
        print(f"CUDA compilation failed: {e}. Trying fallback.", file=sys.stderr)

if wkv_cuda is None:
    try:
        kernels_dir = Path(__file__).resolve().parent / "kernels"
        fallback_path = kernels_dir / "wkv_fallback.py"
        if fallback_path.exists():
            spec = importlib.util.spec_from_file_location(
                "wkv_fallback", str(fallback_path)
            )
        ...
        print(f"Fallback load failed: {e}", file=sys.stderr)
...
class WKV(torch.autograd.Function):
    ...
    def backward(ctx, gy):
        ...
        gw = torch.zeros((B, C), device=w.device).contiguous()
        gu = torch.zeros((B, C), device=w.device).contiguous()
        gk = torch.zeros((B, T, C), device=w.device).contiguous()
        gv = torch.zeros((B, T, C), device=w.device).contiguous()
        half_mode = w.dtype == torch.half
        bf_mode = w.dtype == torch.bfloat16
        wkv_cuda.backward(
            B,
    ...
    def forward(self, x, resolution):
        ...
        for j in range(self.recurrence):
            if j % 2 == 0:
                ...
                zigzag_order = self.get_zigzag_indices(
                    new_h, new_w, start=selected_scheme[0], direction=selected_scheme[1]
                )
...
class Block(nn.Module):
    def __init__(
        ...
        if self.has_inner:
            ...
            self.inner_attn = VRWKV_SpatialMix(
                n_embd=inner_dim, n_layer=None, layer_id=layer_id
            )
            self.inner_norm2 = norm_layer(num_words * inner_dim)
            self.inner_ffn = VRWKV_ChannelMix(
                n_embd=inner_dim, n_layer=None, layer_id=None
            )
        ...
        self.outer_attn = VRWKV_SpatialMix(
            n_embd=outer_dim, n_layer=None, layer_id=layer_id
        )
    ...
    def forward(self, x, outer_tokens, H_out, W_out, H_in, W_in):
        ...
        if self.has_inner:
            ...
            x = x + self.drop_path(
                self.inner_attn(
                    self.inner_norm1(x.reshape(B, N, -1)).reshape(
                        B * N, H_in * W_in, -1
                    ),
            ...
            x = x + self.drop_path(
                self.inner_ffn(
                    self.inner_norm2(x.reshape(B, N, -1)).reshape(
                        B * N, H_in * W_in, -1
                    ),
                    inner_patch_resolution,
                )
...
class Stage(nn.Module):
    def __init__(
        ...
        blocks = []
        drop_path = (
            drop_path if isinstance(drop_path, list) else [drop_path] * num_blocks
        )
...
class PatchMerging2D_sentence(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        x = self.conv(x)
        H, W = math.ceil(math.sqrt(int(N)) / self.stride), math.ceil(
            math.sqrt(int(N)) / self.stride
        )
...
class Stem3D(nn.Module):
    ...
    def forward(self, x):
        ...
        inner_tokens = inner_tokens.reshape(
            B * D, self.inner_dim, inner_tokens.shape[-2], inner_tokens.shape[-1]
        )
        inner_tokens = self.unfold(inner_tokens).transpose(1, 2)
        inner_tokens = inner_tokens.reshape(
            B * D * H_out * W_out, self.inner_dim, H_in * W_in
        ).transpose(1, 2)
...
class PyramidRiR_enc(nn.Module):
    def __init__(
        ...
        self.patch_embed = Stem3D(
            img_size=img_size,
            in_chans=in_chans,
            outer_dim=outer_dims[0],
            inner_dim=inner_dims[0],
        )
        num_patches = self.patch_embed.num_patches
        num_words = self.patch_embed.num_words
        self.pos_embed_sentence = nn.Parameter(
            torch.zeros(1, num_patches, outer_dims[0])
        )
        ...
        for i in range(4):
            if i > 0:
                self.word_merges.append(
                    PatchMerging2D_word(inner_dims[i - 1], inner_dims[i])
                )
                self.sentence_merges.append(
                    PatchMerging2D_sentence(outer_dims[i - 1], outer_dims[i])
                )
            self.stages.append(
                Stage(
                    depths[i],
                    outer_dim=outer_dims[i],
                    inner_dim=inner_dims[i],
                    outer_head=outer_heads[i],
                    inner_head=inner_heads[i],
                    num_patches=num_patches // (2**i) // (2**i),
                    num_words=num_words,
                    mlp_ratio=mlp_ratio,
                    qkv_bias=qkv_bias,
                    qk_scale=qk_scale,
    ...
    def forward_features(self, x):
        inner_tokens, outer_tokens, (H_out, W_out), (H_in, W_in), D = self.patch_embed(
            x
        )
        outputs = []
        ...
        for i in range(4):
            if i > 0:
                inner_tokens = self.word_merges[i - 1](
                    inner_tokens, H_out, W_out, H_in, W_in
                )
                outer_tokens, H_out, W_out = self.sentence_merges[i - 1](
                    outer_tokens, H_out, W_out
                )
...
class Z_RiR(nn.Module):
    def __init__(
        ...
        self.RiR_backbone = PyramidRiR_enc(
            img_size=(192, 192), outer_dims=channels, in_chans=in_channels
        )
