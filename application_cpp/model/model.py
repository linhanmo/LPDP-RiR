...
SESSION_PATH = APP_DIR / "data" / ".session_user"
...
def get_current_user():
    try:
        if not SESSION_PATH.exists():
            return None
        u = SESSION_PATH.read_text(encoding="utf-8").strip()
        return u or None
    except Exception:
        return None
...
def run_unet_dir(input_dir: Path, output_dir: Path):
    if Image is None:
        raise RuntimeError("PIL 未安装，无法进行 U-Net 推理。")
    ...
    print(f"INPUT_DIR: {input_dir.resolve()}", flush=True)
    ...
    try:
        ...
        try:
            ...
            raise RuntimeError(f"无法导入 ACX-UNET：{e}")

    unet = Unet(cuda=False)

    img_files = sorted(list(input_dir.glob("slice_*.png")))
    total = len(img_files)
    ...
    for i, img_path in enumerate(img_files, start=1):
        ...
        idx = _idx_from_name(img_path.stem)
        if idx is None:
            out_name = output_dir / (
                img_path.stem.replace("slice", "proc") + "_unet.png"
            )
        else:
            out_name = output_dir / f"proc_{idx:03d}_unet.png"
...
def main():
    ...
    if args.unet_dir:
        ...
        out_dir = (
            Path(args.unet_out)
            if args.unet_out
            else (base_output_dir / datetime.now().strftime("%Y%m%d_%H%M%S"))
        )
        run_unet_dir(Path(args.unet_dir), out_dir)
        return

    overlay_alpha = float(args.overlay_alpha) if args.overlay_alpha is not None else 0.6
    overlay_alpha = max(0.0, min(1.0, overlay_alpha))
    prob_threshold = (
        float(args.prob_threshold) if args.prob_threshold is not None else 0.5
    )
    prob_threshold = max(0.0, min(1.0, prob_threshold))

    if args.report_only:
        if not args.input:
            parser.error("report_only 模式必须指定 --input")
        if not args.output_dir:
            parser.error("report_only 模式必须指定 --output_dir")

        input_path = Path(args.input)
        weights_path = Path(args.weights) if args.weights else None

        output_save_dir = Path(args.output_dir)
        ensure_dir(output_save_dir)

        if args.input_dir:
            input_save_dir = Path(args.input_dir)
        else:
            guess = APP_DIR / "input" / output_save_dir.name
            input_save_dir = guess if guess.exists() else (APP_DIR / "input")
        ...
        recon_mask = None
        recon_probs = None
        recon_path = output_save_dir / "recon.npz"
        if recon_path.exists():
            try:
                npz = np.load(str(recon_path), allow_pickle=True)
                if "mask" in npz:
                    recon_mask = np.asarray(npz["mask"])
                if "probs" in npz:
                    recon_probs = np.asarray(npz["probs"])
            except Exception:
                recon_mask = None
                recon_probs = None

        lesion_data = None
        lesion_path = output_save_dir / "lesion_data.json"
        if lesion_path.exists():
            try:
                import json

                lesion_data = json.loads(lesion_path.read_text(encoding="utf-8"))
            except Exception:
                lesion_data = None

        ts = (
            output_save_dir.name
            if output_save_dir.name
        ...
        window_width = None
        fg_classes = "1"
        model_info = {"type": "none", "impl": None}
